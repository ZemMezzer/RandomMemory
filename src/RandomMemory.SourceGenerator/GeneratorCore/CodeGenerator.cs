    ...
    internal static class CodeGenerator
    {
        ...
        public static GenerationContext CreateGenerationContext(TypeDeclarationSyntax classDecl, AttributeData memoryTableAttribute, DiagnosticReporter reporter)
        {
            ...
            var usingStrings = root.DescendantNodes()
                ...
                .Concat(new[] { "using RandomMemory", "using RandomMemory.Validation", "using System", "using System.Collections.Generic" })
                .Concat(ns)
                .Select(x => x.Trim(';') + ";")
                .Distinct()
        ...
        static (PrimaryKey, List<SecondaryKey>, PropertyDeclarationSyntax)? ExtractPropertyAttribute(PropertyDeclarationSyntax property, DiagnosticReporter reporter)
        {
            ...
            foreach (var attrList in property.AttributeLists)
            {
                ...
                foreach (var attr in attrList.Attributes)
                {
                    ...
                    if (attrName == "PrimaryKey" || attrName == "RandomMemory.PrimaryKey")
                    {
                    ...
                    else if (attrName == "SecondaryKey" || attrName == "RandomMemory.SecondaryKey")
                    {
                    ...
                    else if (attrName == "NonUnique" || attrName == "RandomMemory.NonUnique")
                    {
                        hasNonUnique = true;
                    }
                    else if (attrName == "StringComparisonOption" || attrName == "RandomMemory.StringComparisonOption")
                    {
