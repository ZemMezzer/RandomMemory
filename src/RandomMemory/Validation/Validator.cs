...
namespace RandomMemory.Validation
{
    internal class Validator<T> : IValidator<T>
    {
        ...
        public RandomMemory.Validation.ValidatableSet<T> GetTableSet()
        {
            return new RandomMemory.Validation.ValidatableSet<T>(database.GetTable<T>(), resultSet, pkName, pkSelector);
        }
